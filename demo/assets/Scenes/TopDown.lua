...
--
function TopDown:checkInputs()
	if (self:checkKey(KeyCode.A) or self:checkKey(KeyCode.DPAD_LEFT)) then 
		self.player:setMoveVecX(-1)
	end
	if (self:checkKey(KeyCode.D) or self:checkKey(KeyCode.DPAD_RIGHT)) then 
		self.player:setMoveVecX(1)
	end
	if (self:checkKey(KeyCode.W) or self:checkKey(KeyCode.DPAD_UP)) then 
		self.player:setMoveVecY(-1)
	end
	if (self:checkKey(KeyCode.S) or self:checkKey(KeyCode.DPAD_DOWN)) then 
		self.player:setMoveVecY(1)
	end
...
function TopDown:onDrawUI(dt)
	...
	self.bg:setPosition(x - cam.w / 2 - 256, y - cam.h / 2 - 256)
end